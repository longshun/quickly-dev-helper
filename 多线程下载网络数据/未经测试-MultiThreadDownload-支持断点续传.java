package com.longshun.animationdemo.zhufengfm.zhufengfm.utils;
...
/**
 * Created by longShun on 2015/10/28.
 * 多线程下载网络数据，支持断点下载
 */
        ...
        public void run()
        {
            ...
            try
            {
                ...
                if (conn.getResponseCode() == 206)
                {
                    ...
                    raf.seek(startPos);
                    //用来保存已经下载好的内容的长度
                    long currentDownloadPos = startPos;

                    while ((len = is.read(buf)) != -1)
                    {
                        raf.write(buf, 0, len);
                        currentDownloadPos += len;
                    }
                    //判断是否下载完该区间的文件,未完成的话，继续开线程下载
                    if(currentDownloadPos < endPos){
                        long restartPos = currentDownloadPos+1;
                        new DownloadThread(new URL(urlStr), localFile, restartPos, endPos).start();
                        this.isFinish = false;
                    }else {
                        raf.close();
                        is.close();
